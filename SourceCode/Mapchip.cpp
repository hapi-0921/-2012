...
Map::Map()
{
	...
	sprpost = sprite_load(L"./Data/Images/Post.png");
	sprbord = sprite_load(L"./Data/Images/Signboard.png");
	...
	sprmap4gray = sprite_load(L"./Data/Images/ParkDark.png");
	sprmap5 = sprite_load(L"./Data/Images/House.png");
	sprmap5gray = sprite_load(L"./Data/Images/HouseDark.png");
	sprmap6 = sprite_load(L"./Data/Images/Piano.png");
	sprmap6gray = sprite_load(L"./Data/Images/PianoDark.png");
...
void Map::Update()
{
	if (!goal)
	{
		...
		{
			if (c.move.pos.y < Y - 300 ||
				c.move.pos.y > Y + map.size() * chipSize + 300 ||
				c.move.pos.x < X - 300 ||
				c.move.pos.x > X + map[0].size() * chipSize + 300)
			{
				c.move.active = false;
				c.move.pos.x = 2000;
				c.move.pos.y = 2000;
		...
		if (!c.move.active)
		{
			c.move.carsporntimer++;
		}
		if (c.move.carsporntimer >= 600)
		{
			c.carmove = false;
			c.move.carsporntimer = 0;
...
void Map::Move(MoveObject& obj, RoadInfo& info)
{
	...
	if (c.move.cartocollide)
	{
		...
		{
			obj.pos.x += obj.dirX * obj.speed * 2;
			...
			case 0:
				for (int i = 0; i < map[0].size(); i++)
				{
						...
						block[0][j].RotationCount % 2 == 0)
					{
						...
						if (CanMove(obj, info, DOWN))
						{
							DangerX = j * chipSize + X + MapCenter;
							DangerY = Y - 50;
							Dangerangle = 180;
							c.warning = true;
							c.warningTimer = 300; // 1秒待つ
							c.spawnType = 0;
							c.spawnIndex = j;
			...
			case 1:
				for (int i = 0; i < map[0].size(); i++)
				{
						...
						block[map.size() - 1][j].RotationCount % 2 == 0)
					{
						...
						{
							DangerX = j * chipSize + X + MapCenter;
							DangerY = Y + map.size() * chipSize + 50;
							Dangerangle = 0;
							c.warning = true;
							c.warningTimer = 300;
							c.spawnType = 1;
			...
			case 2:
				for (int i = 0; i < map.size(); i++)
				{
						...
						block[j][map[0].size() - 1].RotationCount % 2 != 0)
					{
						...
						if (CanMove(obj, info, LEFT))
						{
							DangerX = X + map[0].size() * chipSize + 50;
							DangerY = j * chipSize + Y + MapCenter;
							Dangerangle = 270;
							c.warning = true;
							c.warningTimer = 300;
							c.spawnType = 2;
	...
	for (int i = 0;i < map.size();i++) {				//ブロック描画
		for (int j = 0;j < map[i].size();j++) {
			...
			case 1:
				...
				else if (block[i][j].stop == 3)
				{
					sprite_render(sprpost,
						posX, posY,
						scale, scale,
						0, 0,
						64, 64,
